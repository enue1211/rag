...
ORIGIN_CSV = os.path.join(ORIGIN_DIR, "company_manual.csv")  # 원본
...
def load_or_create_index_no_ui() -> tuple[pd.DataFrame, np.ndarray]:
    ...
    if "content" not in df.columns:
        raise ValueError("company_manual.csv에는 최소 'content' 컬럼이 필요합니다.")
...
st.caption("버튼 클릭 시.. embedding 디렉토리를 삭제하고, company_manual.csv 기반으로 em01.csv와 em01.npy를 재생성")
